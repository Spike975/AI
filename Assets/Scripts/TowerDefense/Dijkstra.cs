            ...
            for (int i = 0; i < close.Length; i++)
            {
                ...
                if (temp.cur.GetComponent<NodeScript>().around[0] != null)
                {
                    ...
                    for (int x = 0; x < closeLength; x++)
                    {
                        ...
                        {
                            if (close[x].f > temp.cur.GetComponent<NodeScript>().weight + temp.f)
                            {
                    ...
                    for (int x = 0; x < openLength; x++)
                    {
                        ...
                        {
                            if (open[x].f > temp.cur.GetComponent<NodeScript>().weight + temp.f)
                            {
    ...
    private void OnDrawGizmos()
    {
        ...
        if (path != null)
        {
            for (int i = 0; i < path.Length; i++)
            {
                if (path[i] != null)
                {
                    Gizmos.DrawSphere(new Vector3(path[i].transform.position.x+.5f, path[i].transform.position.y+1, path[i].transform.position.z-.5f), .25f);
                }
                else
                {
                    break;
